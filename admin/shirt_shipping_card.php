<div class="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm page-break relative label-card group hover:shadow-lg hover:border-yellow-300 transition-all duration-300" data-id="<?php echo $row['id']; ?>" data-type="shirt">
    ...
    <div class="type-badge shirt">สั่งเสื้อ</div>

    <!-- Order Number -->
    <div class="order-id absolute top-4 right-4 text-yellow-500 font-mono text-xs font-bold tracking-widest"><?php echo htmlspecialchars($row['order_number']); ?></div>

    <!-- Sender -->
    <div class="sender-section text-xs text-slate-500 mb-4 border-b border-dashed border-slate-200 pb-3 pt-6 relative">
        <div class="absolute -left-2 top-0 w-1 h-full bg-yellow-100 rounded-r-full no-print"></div>
    ...
    <div class="receiver-section pl-4 mb-4 relative">
        ...
        ?>
        <p class="address-text text-slate-600 text-md leading-relaxed font-medium screen-address" style="-webkit-line-clamp:3; display:-webkit-box; -webkit-box-orient:vertical; overflow:hidden;" title="<?php echo htmlspecialchars($raw_address); ?>"><?php echo htmlspecialchars($compact_address ?: 'ไม่มีที่อยู่'); ?></p>
        <p class="address-text print-address text-slate-600 text-md leading-relaxed font-medium"><?php echo nl2br(htmlspecialchars($formatted_address ?: 'ไม่มีที่อยู่')); ?></p>
        <p class="phone-text text-slate-600 mt-3 font-bold flex items-center gap-2 bg-slate-50 inline-block px-2 py-1 rounded-lg border border-slate-100"><i class="fas fa-mobile-alt text-yellow-500 no-print"></i><?php echo htmlspecialchars($row['phone']); ?></p>
    </div>

    <!-- Order Info (Footer) -->
    <div class="footer-section bg-yellow-50 -mx-6 -mb-6 p-4 mt-auto flex justify-between items-center border-t border-yellow-100 rounded-b-2xl">
        <div>
            <span class="block text-[10px] text-slate-400 uppercase font-bold mb-0.5">รายการเสื้อ</span>
            <span class="category-text text-xs font-bold text-slate-600 truncate max-w-[180px] block bg-white px-2 py-1 rounded border border-slate-200 shadow-sm" title="<?php echo htmlspecialchars($row['shirt_sizes']); ?>"><?php echo htmlspecialchars($row['shirt_sizes']); ?></span>
        </div>
        <div class="text-right">
            <span class="block text-[10px] text-slate-400 uppercase font-bold mb-0.5">จำนวน</span>
            <span class="size-text inline-block bg-yellow-100 border border-yellow-200 px-2 py-1 rounded text-yellow-700 font-bold text-sm shadow-sm min-w-[40px] text-center"><?php echo htmlspecialchars($row['shirt_quantity']); ?> ตัว</span>
        </div>
    </div>
</div>
